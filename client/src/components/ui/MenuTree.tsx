...
const MenuTreeItem = ({ title, icon: Icon, to, isActive,  children, depth }: MenuTreeItemProps) => {
  ...
  return (
    <>
      {!hasChildren && to ? (
        <Link className={className} to={to}>{itemContent}</Link>
      ) : (
        <div
...
const MenuTree = ({ data, pathname }: MenuTreeProps) => {
  return (
    <ul>
      {data.map((datum, i) => (
        <MenuTreeItem key={i} depth={1.5} {...datum} isActive={pathname === datum.to} />
      ))}
    </ul>
  );
};
